...
This project was built using `@wordpress/scripts` as the build toolchain and uses modern best practices, including React, ESNext, and SCSS.
...
- **Live Editor Preview:** Uses `useSelect` and `getEntityRecords` from `@wordpress/data` to fetch real post data via the WordPress REST API, providing a live content preview directly on the editor canvas.
...
- **Inspector Controls:** Button text and URL are configured via a dedicated sidebar panel using `TextControl` and `URLInput`, including WordPress-native URL autocomplete for internal links.
